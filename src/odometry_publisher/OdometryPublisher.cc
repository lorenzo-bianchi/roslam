    ...
    EventManager &/*_eventMgr*/)
{
  ...
  // Setup odometry
  std::string odomTopic{"/model/" + this->dataPtr->model.Name(_ecm) + "/odometry"};
  ...
  // {
  //   ignerr << "Failed to generate valid TF topic from [" << tfTopic << "]" << std::endl;
  // }
  // else
  // {
  //   this->dataPtr->tfPub = this->dataPtr->node.Advertise<msgs::Pose_V>(tfTopicValid);
  //   ignmsg << "OdometryPublisher publishing Pose_V (TF) on [" << tfTopicValid << "]" << std::endl;
  // }
}
    ...
    const ignition::gazebo::EntityComponentManager &_ecm)
{
  ...
  // We cannot estimate the speed if the time interval is zero (or near zero).
...
IGNITION_ADD_PLUGIN_ALIAS(OdometryPublisher, "custom::OdometryPublisher")
