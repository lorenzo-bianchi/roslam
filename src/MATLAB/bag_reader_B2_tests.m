...
for i = 1:length(uwb_topics_names)
    ...
    xlabel('time [s]');
    ylabel('distance [m]');
    legend('Anchor 5', 'Anchor 6', 'Anchor 7', 'Anchor 8', 'Location', 'southwest');
...
T = tiledlayout(n_robots, 1, 'TileSpacing', 'Compact', 'Padding', 'Compact');
...
for i = 1:n_robots
    ...
    t = tiledlayout(T, 1, 3, 'TileSpacing', 'Compact', 'Padding', 'Compact');
    t.Layout.Tile = i;
...
T = tiledlayout(length(est_pose_topics_names), 1, 'TileSpacing', 'Compact', 'Padding', 'Compact');
...
for i = 1:length(tests_pre)
    ...
    for k = 1:len_bag
        ...
        [R, t, inl] = ransacRototranslation(xy_lnds, pos_anchors, 1000, 0.1, 7);
        if isempty(inl)
