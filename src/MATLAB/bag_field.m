...
figure;
set(gcf, 'Position', [100, 100, 550, 800]);
hold on;
grid on;
for idx = 1:num_anchors+num_points
    if idx <= num_anchors
        plot(z_opt(idx, 1), z_opt(idx, 2), 'rh', 'Linewidth', 2, 'MarkerSize', 14);
        text(z_opt(idx, 1), z_opt(idx, 2)+0.05, ['A' num2str(idx)], 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 16, 'FontWeight', 'bold', 'Color', 'red');
    else
        if idx ~= 14 && idx ~= 15
            plot(z_opt(idx, 1), z_opt(idx, 2), 'xk', 'Linewidth', 2, 'MarkerSize', 14);
            text(z_opt(idx, 1), z_opt(idx, 2)+0.05, names_points(idx-num_anchors), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 16, 'FontWeight', 'bold');
        end
    end
end
...
ylabel('y [m]')
axis equal;
xlim([min(z_opt(:,1))-0.3, max(z_opt(:,1))+0.3]);
ylim([min(z_opt(:,2))-0.5, max(z_opt(:,2))+0.5]);
set(gca, 'FontSize', 16, 'FontWeight', 'bold');

print(gcf, 'figures/test_field_B1.eps', '-depsc2', '-vector', '-r300', '-noui');
