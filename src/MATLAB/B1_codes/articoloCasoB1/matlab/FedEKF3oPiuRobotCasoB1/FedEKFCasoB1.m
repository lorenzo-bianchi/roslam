...
xRHat0(1) = robots(1).xVett(1);
yRHat0(1) = robots(1).yVett(1);
...
for indFedEKF = 1:nFedEKF
    ...
    % Robot 1:
    FedEKFs(indFedEKF).xHatSLAM(1:3:1+3*(nPhi-1),1) = xRHat0(1);
    FedEKFs(indFedEKF).xHatSLAM(2:3:2+3*(nPhi-1),1) = yRHat0(1);
    FedEKFs(indFedEKF).xHatSLAM(3:3:3+3*(nPhi-1),1) = possibiliPhi;
    PtriplettaRobot1 = zeros(3,3);
    PtriplettaRobot1(3,3) = sigmaPhi^2;
    
    % Robot 2:
    FedEKFs(indFedEKF).xHatSLAM(3*nPhi+1:3:6*nPhi-2,1) = xRHat0(2);
    FedEKFs(indFedEKF).xHatSLAM(3*nPhi+2:3:6*nPhi-1,1) = yRHat0(2);
    ...
    FedEKFs(indFedEKF).xHatSLAM(6*nPhi+1:3:9*nPhi-2,1) = xRHat0(3);
    FedEKFs(indFedEKF).xHatSLAM(6*nPhi+2:3:9*nPhi-1,1) = yRHat0(3);
    ...
    for indRobot = 4:nRobot
        xPunti = xRHat0(1:indRobot-1);
        yPunti = yRHat0(1:indRobot-1);
        raggiCerchi = misuraRangeTraRobot(1:indRobot-1,indRobot);
        [xRHat0(indRobot),yRHat0(indRobot),allineatiVett(indRobot-3)] = trovaPosTag(xPunti,yPunti,raggiCerchi);
        % for jndRobot = 1:indRobot
        %     plot(robots(jndRobot).xVett(1),robots(jndRobot).yVett(1),'cs')
        % end
        % pause
        FedEKFs(indFedEKF).xHatSLAM(3*(indRobot-1)*nPhi+1:3:3*indRobot*nPhi-2,1) = xRHat0(indRobot);
        FedEKFs(indFedEKF).xHatSLAM(3*(indRobot-1)*nPhi+2:3:3*indRobot*nPhi-1,1) = yRHat0(indRobot);
    ...
    % Qui si potrebbe ottimizzare il poligono dei robot in base alle
    % distanze misurate partendo dall'inizializzazione qui sopra e poi
    % facendo un'ottimizzazione basata sulle misure di distanza. Un'altra
    % cosa che si potrebbe fare  scegliere il terzo robot come quello che 
    % massimizza la non collinearit coi robot 1 e 2, in modo che poi i robot 
    % successivi vengano individuati meglio

    % Inzializzazione landmark
    xPunti = xRHat0(1:nRobot);
    yPunti = yRHat0(1:nRobot);
    for indTag = 1:nTag
        % indFedEKF
        % indTag
        raggiCerchi = misuraRangeTagRobot(:,indTag);
        [xT,yT,allineatiVett(nRobot-3+indTag)] =  trovaPosTag(xPunti,yPunti,raggiCerchi);
        % plot(cTag(indTag,1),cTag(indTag,2),'cd')
        % pause
        indici = nRobot*3*nPhi+2*indTag-1:nRobot*3*nPhi+2*indTag;
        FedEKFs(indFedEKF).xHatSLAM(indici) = [xT,yT]';
