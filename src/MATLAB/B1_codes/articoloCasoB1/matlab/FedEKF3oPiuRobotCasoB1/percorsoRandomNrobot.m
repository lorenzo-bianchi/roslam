...
for indRobot = 1:nRobot
    ...
    robots(indRobot).xVett(1) = 30+(L-60)*rand; %(L-2*clearance)*rand + clearance; %100+clearance*cos(angoloRand);%clearance;% + rand*(L-2*clearance);
    robots(indRobot).yVett(1) = 30+(L-60)*rand; %(L-2*clearance)*rand + clearance; %100+clearance*sin(angoloRand);%70;% clearance + rand*(L-2*clearance);
    if (indRobot==2)
        distanzaR1R2 = sqrt((robots(1).xVett(1)-robots(2).xVett(1))^2+(robots(1).yVett(1)-robots(2).yVett(1))^2);
        while distanzaR1R2 < 50
            robots(indRobot).xVett(1) = 30+(L-60)*rand; %(L-2*clearance)*rand + clearance; %100+clearance*cos(angoloRand);%clearance;% + rand*(L-2*clearance);
            robots(indRobot).yVett(1) = 30+(L-60)*rand; %(L-2*clearance)*rand + clearance; %100+clearance*sin(angoloRand);%70;% clearance + rand*(L-2*clearance);
            distanzaR1R2 = sqrt((robots(1).xVett(1)-robots(2).xVett(1))^2+(robots(1).yVett(1)-robots(2).yVett(1))^2);
        end
    end
    robots(indRobot).thetaVett(1) = 2*pi*rand; %0;%360*rand*gradi; % angolo in radianti: gradi e' il fattore di conversione
