...
probMisura_ij = zeros(nPhi,1); % viene chiamato elle_ij nell'articolo di ICRA 2010
probMisuraX_ij = zeros(nPhi,1); % viene chiamato elle_ij nell'articolo di ICRA 2010
probMisuraY_ij = zeros(nPhi,1); % viene chiamato elle_ij nell'articolo di ICRA 2010
...
stdMisuraMedia = 5;
kCond = zeros(nPassi,1);
unaSolaIpo = zeros(nRobot,nTag);
condivisione = 0;
...
RsX = zeros(nTag*nPhi,nTag*nPhi);
RsY = zeros(nTag*nPhi,nTag*nPhi);

nDistanze = nTag*(nTag-1)/2;
distanzaIJmedia = zeros(nDistanze,1);
distanzaIJvett  = zeros(nRobot,1);

% % PROVA per VEDERE che succede se uso distanze esatte invece di quelle
% % medie stimate
% indice = 0;
% for indTag = 1:nTag-1
%     for jndTag = indTag+1:nTag
%         indice = indice + 1;
%         distanzaIJmedia(indice) = sqrt((cTag(indTag,1)-cTag(jndTag,1))^2+(cTag(indTag,2)-cTag(jndTag,2))^2);
%     end
% end
    ...
    for indRobot = 1:nRobot
        ...
        if mod(k+1,Nstep) == 1 || (Nstep == 1) 
            ...
            for indTag = 1:nTag
                ...
                if numel(find(robots(indRobot).pesi(indTag,:)>.001)) == 1
                    unaSolaIpo(indRobot,indTag) = 1;
                end
            end
        ...
        if DISEGNA && mod(k,5) == 1 %(k>300 || mod(k,5) == 1)
            figure(indRobot)
            hold off
            disegnaFig
            if condivisione
                title([num2str(k) ' - condivisione ATTIVA'])
            else
                title(num2str(k))
            end
...
%             pause(.01)
    ...
    % Condivisione stime landmark (da considerare come misure): 
    condivisioneMisureCasoB2;
    
    % if DISEGNA && kCond(k) == 1 && mod(k,5) == 1 %(k>300 || mod(k,5) == 1)        
    %     for indRobot = 1:nRobot
    %         figure(indRobot)
    %         for indTag = 1:nTag-1
    %             for jndTag = indTag+1:nTag
    %                 plot([cTag(indTag,1) cTag(jndTag,1)]/100,[cTag(indTag,2) cTag(jndTag,2)]/100,'g--')
    %             end
    %         end
    %     end
    % end
...
for indRobot = 1:nRobot
    ...
    for indTag = 1:nTag-1
        for jndTag = indTag+1:nTag
            ...
            for k = 1:nPassi
                robots(indRobot).distanzeInterTagStimateStoria(indice,k) = sqrt((robots(indRobot).xHatTagStoria(indTag,k)-robots(indRobot).xHatTagStoria(jndTag,k)).^2+(robots(indRobot).yHatTagStoria(indTag,k)-robots(indRobot).yHatTagStoria(jndTag,k)).^2);
            end
        end
    end
...
if DISEGNA
    ...
    indice = 3;
    figure
    plot(robots(indRobot).distanzeInterTagVere(indice)*ones(1,nPassi))
    hold on
    plot(robots(indRobot).distanzeInterTagStimateStoria(indice,:),'r.--')
    kInizCond = min(find(kCond));
    if isempty(kInizCond) == 0
        plot(kInizCond*[1 1],[0 robots(indRobot).distanzeInterTagVere(indice)*1.2],'k--')
    end
    axis([0 nPassi 0 robots(indRobot).distanzeInterTagVere(indice)*1.2])
end