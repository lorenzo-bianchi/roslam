...
probMisuraX_ij = zeros(nPhi,1); % quello che viene chiamato elle_ij nell'articolo di ICRA 2010
probMisuraY_ij = zeros(nPhi,1); % quello che viene chiamato elle_ij nell'articolo di ICRA 2010
...
xHatTagMedia = zeros(nTag,nPassi);
yHatTagMedia = zeros(nTag,nPassi);
xHatTagStd = zeros(nTag,nPassi);
yHatTagStd = zeros(nTag,nPassi);
...
%             pause(.01)
    ...
    % Calcolo media e covarianza stima coordinate landmark 
    for indTag = 1:nTag
        vettoreX = [];
        vettoreY = [];
        for indRobot = 1:nRobot
            vettoreX = [vettoreX robots(indRobot).xHatTagStoria(indTag,k+1)];
            vettoreY = [vettoreY robots(indRobot).yHatTagStoria(indTag,k+1)];
        end
        xHatTagMedia(indTag,k+1) = mean(vettoreX);
        yHatTagMedia(indTag,k+1) = mean(vettoreY);
        xHatTagStd(indTag,k+1) = std(vettoreX);
        yHatTagStd(indTag,k+1) = std(vettoreY);
    end
    
    % Disegno stime medie
    if DISEGNA && mod(k,5) == 1 %(k>300 || mod(k,5) == 1)
        for indTag = 1:nTag
            plot(xHatTagMedia(indTag,k+1)/100,yHatTagMedia(indTag,k+1)/100,'go','LineWidth',3)
        end
        pause(.01)
    end
    
    % Condivisione stime landmark (da considerare come misure): 
    condivisioneMisureCasoA;
