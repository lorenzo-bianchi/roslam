...
nPhi = 8;
nPassi = 1654;
robotScelto = 3; % e' il robot dei tre che si vuole testare
...
% Definisco un insieme di robot (in realta' e' solo uno)
...
if robotScelto == 1
    robots(1).xVett(1) =  0.0905*100;
    robots(1).yVett(1) = 2.6885*100;
    robots(1).thetaVett(1) = 0; % e' gia' zero ma giusto per chiarezza
    % cosi' hanno tutti lo stesso numero di elementi e i tempi molto vicini
    robots(1).uRe = 100*odoRuote(9:nPassi+8,2);
    robots(1).uLe = 100*odoRuote(9:nPassi+8,3);
end
if robotScelto == 2
    robots(1).xVett(1) =  0.0605*100;
    robots(1).yVett(1) = 1.6437*100;
    robots(1).thetaVett(1) = -pi/2;
    robots(1).uRe = 100*odoRuote(1:nPassi,2);
    robots(1).uLe = 100*odoRuote(1:nPassi,3);
end
if robotScelto == 3
    robots(1).xVett(1) =  1.7309*100;
    robots(1).yVett(1) = 0.7688*100;
    robots(1).thetaVett(1) = pi/2;
    robots(1).uRe = 100*odoRuote(1:nPassi,2);
    robots(1).uLe = 100*odoRuote(1:nPassi,3);
end
...
for k = 1:nPassi-1
    deltaRho = (robots(1).uRe(k)+robots(1).uLe(k))/2;
    deltaTheta = (robots(1).uRe(k)-robots(1).uLe(k))/dVera;
    robots(1).xVett(k+1) = robots(1).xVett(k) + deltaRho*cos(robots(1).thetaVett(k));
    robots(1).yVett(k+1) = robots(1).yVett(k) + deltaRho*sin(robots(1).thetaVett(k));
    robots(1).thetaVett(k+1) = robots(1).thetaVett(k) + deltaTheta;
end
...
if robotScelto == 1
    ...
    for jndTag = 1:nTag
        misureTutte(jndTag,:) = 100*matrice(:,jndTag+1);
    end
end    
...
for indRobot = 1:nRobot
    ...
    if displayErrori
        ...
        disp('RELATIVI:')
        disp('Distanze del robot dai tag da ric. odometrica (sopra) e stimate (sotto)')
...
if DISEGNA
    ...
    % axis([0 2 -0 2])
    plot(robots(indRobot).xHatSLAM(1,end)/100,robots(indRobot).xHatSLAM(2,end)/100,'o')
    for indTag = 1:nTag
        indiceStartIndTag = 3+3*(indTag-1)+sum(nIpoTag(1:indTag-1));
