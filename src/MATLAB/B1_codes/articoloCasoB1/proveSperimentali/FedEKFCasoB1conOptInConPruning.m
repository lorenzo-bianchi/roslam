...
sigmaDistanza = 10; % std in cm della misura di range
...
for indRobot = 1:nRobot
    ...
    odoRuote = roslam_data.wheels_odometry{indRobot};
    if indRobot == 1
        tempiOdoRobot = odoRuote(:,1); % prendo come tempi quelli del robot 1, gli altri sono simili
    end
    if indRobot == 1
        robots(indRobot).xVett(1) =  0.0905*100;
        robots(indRobot).yVett(1) = 2.6885*100;
        robots(indRobot).thetaVett(1) = 0; % e' gia' zero ma giusto per chiarezza
        % cosi' hanno tutti lo stesso numero di elementi e i tempi molto vicini
        robots(indRobot).uRe = 100*odoRuote(9:nPassi+8,2);
        robots(indRobot).uLe = 100*odoRuote(9:nPassi+8,3);
    end
    if indRobot == 2
        robots(indRobot).xVett(1) =  0.0605*100;
        robots(indRobot).yVett(1) = 1.6437*100;
        robots(indRobot).thetaVett(1) = -pi/2;
        robots(indRobot).uRe = 100*odoRuote(1:nPassi,2);
        robots(indRobot).uLe = 100*odoRuote(1:nPassi,3);
    end
    if indRobot == 3
        robots(indRobot).xVett(1) =  1.7309*100;
        robots(indRobot).yVett(1) = 0.7688*100;
        robots(indRobot).thetaVett(1) = pi/2;
        robots(indRobot).uRe = 100*odoRuote(1:nPassi,2);
        robots(indRobot).uLe = 100*odoRuote(1:nPassi,3);
    end
    ...
    for k = 1:nPassi-1
        deltaRho = (robots(indRobot).uRe(k)+robots(indRobot).uLe(k))/2;
        deltaTheta = (robots(indRobot).uRe(k)-robots(indRobot).uLe(k))/dVera;
        robots(indRobot).xVett(k+1) = robots(indRobot).xVett(k) + deltaRho*cos(robots(indRobot).thetaVett(k));
        robots(indRobot).yVett(k+1) = robots(indRobot).yVett(k) + deltaRho*sin(robots(indRobot).thetaVett(k));
        robots(indRobot).thetaVett(k+1) = robots(indRobot).thetaVett(k) + deltaTheta;
    end


end

% Ogni colonna ha misure: R1-Lj, R2-Lj, R3-Lj, R1-R2, R1-R3, R2-R3.
misureTutte = zeros(15,nPassi/2);
% Ri-Lj
indRobot = 1;
matrice = roslam_data.uwb_anchors_distances{indRobot,1};
for jndTag = 1:nTag
    misureTutte(jndTag,:) = 100*matrice(10:end,jndTag+1);
end
for indRobot = 2:3
    matrice = roslam_data.uwb_anchors_distances{indRobot,1};
    for jndTag = 1:nTag
        misureTutte(4*(indRobot-1)+jndTag,:) = 100*matrice(:,jndTag+1);
    end
end    
% Ri-Rj
matrice = roslam_data.uwb_inter_robots_distances{1,1};  
misureTutte(13,:) = 100*matrice(7:end,3); % misureR1R2
misureTutte(14,:) = 100*matrice(7:end,4); % misureR1R3 
matrice = roslam_data.uwb_inter_robots_distances{2,1};  
misureTutte(15,:) = 100*matrice(2:end,4); % misureR2R3 

% Aggiungendo il byas osservato da Lorenzo (vedi email 28dic2024 ore 19:09)
% misureTutte = misureTutte + 10;
% Aggiungendo invece il bias ricavato dale posizioni iniziali dei robot:
% misureTutte(2,:) = misureTutte(2,:) + 8;
% misureTutte(3,:) = misureTutte(3,:) + 10;
% misureTutte(4,:) = misureTutte(4,:) + 18;
% misureTutte(5,:) = misureTutte(5,:) + 15;
% misureTutte(6,:) = misureTutte(6,:) + 3;
% misureTutte(7,:) = misureTutte(7,:) + 5;
% misureTutte(8,:) = misureTutte(8,:) - 35;
% misureTutte(9,:) = misureTutte(9,:) + 15;
% misureTutte(10,:) = misureTutte(10,:) + 8;
% misureTutte(11,:) = misureTutte(11,:) + 15;
% misureTutte(12,:) = misureTutte(12,:) + 15;
% misureTutte(13,:) = misureTutte(13,:) + 27;
% misureTutte(14,:) = misureTutte(14,:) - 15;
% misureTutte(15,:) = misureTutte(15,:) + 18;
...
% Misure iniziali
% Siccome uso una misura ogni Nstep = 2 passi, questo e' l'indice della misura da usare:
kMisura = 1;
misuraRangeTraRobot = zeros(nRobot,nRobot);
for indRobot = 1:nRobot-1
    for jndRobot = indRobot+1:nRobot
        misuraRangeTraRobot(indRobot,jndRobot) = misureTutte(12+(indRobot-1)+(jndRobot-1),kMisura);
    end
end
misuraRangeTagRobot = zeros(nRobot,nTag);
for indRobot = 1:nRobot
    for indTag = 1:nTag
        misuraRangeTagRobot(indRobot,indTag) = misureTutte(4*(indRobot-1)+indTag,kMisura);
    end
end
    ...
    for indFedEKF = 1:nFedEKF
        ...
        if indBuoniFedEKF(indFedEKF)
            ...
            if mod(k+1,Nstep) == 1 || (Nstep == 1) 
                ...
                % Misure di range tra robot
                for indRobot = 1:nRobot-1
                    for jndRobot = indRobot+1:nRobot
                        % Se e' NaN usa la misura precedente
                        if ~isnan(misureTutte(12+(indRobot-1)+(jndRobot-1),kMisura))
                            misuraRangeTraRobot(indRobot,jndRobot) = misureTutte(12+(indRobot-1)+(jndRobot-1),kMisura);
                        end
                    end
                end
                for indRobot = 1:nRobot
                    for indTag = 1:nTag
                        % Se e' NaN usa la misura precedente
                        if ~isnan(misureTutte(4*(indRobot-1)+indTag,kMisura))
                            misuraRangeTagRobot(indRobot,indTag) = misureTutte(4*(indRobot-1)+indTag,kMisura);
                        end
                    end
                end
                    ...
                    for jndRobot = indRobot+1:nRobot
                        ...
                        for indIpoRi = 1:nIpoRobot(indRobot)
                            ...
                            for indIpoRj = 1:nIpoRobot(jndRobot)
                                ...
                                probMisura(nIpoRobot(jndRobot)*(indIpoRi-1)+indIpoRj) = exp(-deltaMisura^2/(2*(sigmaDistanzaModello^2/2)));
                                % indRobot 
                                % jndRobot
                                % indIpoRi
                                % indIpoRj
...
distanzeFinaliDaiTagStimate = zeros(nRobot,nTag);
for indRobot = 1:nRobot
    for indTag = 1:nTag
        distanzeFinaliDaiTagStimate(indRobot,indTag) = sqrt((xHatRobotEnd(indRobot)-cHatTagEnd(indTag,1))^2+(yHatRobotEnd(indRobot)-cHatTagEnd(indTag,2))^2);
    end 
end
...
if displayErrori
    ...
    disp('Distanze tra i robot da ric. odometrica (sopra) e stimate (sotto)')
    [distanzeFinaliRobotVere';distanzeFinaliRobotStimate']
    disp('Distanze tra i robot e i landmark da ric. odometrica (sopra) e stimate (sotto)')
    [distanzeFinaliTagVere'; distanzeFinaliTagStimate']
    disp('Matrice distanze tra robot e landmark finali stimate')
    distanzeFinaliDaiTagStimate
...
if DISEGNA
    ...
    for indRobot = 1:nRobot
        ...
        plot(xRobotFinale/100,yRobotFinale/100,'ks')
    end
    % axis([0 2 -0 2])
    ...
    for indTag = 1:nTag
        ...
        xTag = FedEKFs(indFedEKF).xHatSLAM(indiceStartTagi+1,k+1);
        yTag = FedEKFs(indFedEKF).xHatSLAM(indiceStartTagi+2,k+1);
        plot(xTag/100,yTag/100,'rs','LineWidth',3) 
    end
    grid on
end