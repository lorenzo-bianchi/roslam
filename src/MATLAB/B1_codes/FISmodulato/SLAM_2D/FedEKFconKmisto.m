% FedEKF in cui nel passo di correzione correggo prima la posa del robot e 
% le variabili comuni (xi, yi, rhoi) delle varie ipotesi dei vari landmark 
% (processate una alla volta), e poi correggo l'angolo phiij di ogni
% ipotesi, usando eventualmente un KalmanGain diverso nei due passi.
...
for k = 1:nPassi-1
    ...
    if mod(k+1,Nstep) == 1 || (Nstep == 1) 
        ...
        for indTag = 1:nTag
            ...
            % Calcolo Rs: posso decidere in quale passo cambiare la Rs
            if k > 1000
                for jndPhi = 1:nPhi
                    phi_ij = xTemp(6+(3+nPhi)*(indTag-1)+jndPhi);
                    cosPhi_ij = cos(phi_ij);
