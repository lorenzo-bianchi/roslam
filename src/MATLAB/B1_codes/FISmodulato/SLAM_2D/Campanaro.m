...
storiaInnovazione = zeros(nPassi,1);
...
for k = 1:nPassi-1
    ...
    if mod(k+1,Nstep) == 1 || (Nstep == 1) 
        ...
        [massimo indMax] = max(abs(innovazioneMinTag));
        storiaInnovazione(k+1) = innovazioneMinTag(indMax);

        if k > 50
            passoMin = max(1,k-49);
            mediaInn = mean(storiaInnovazione(passoMin:k+1));
            if abs(mediaInn) > 10
                disp('outlier')
                return
            end
