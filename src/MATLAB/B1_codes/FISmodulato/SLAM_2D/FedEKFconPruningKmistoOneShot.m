...
% Inizializzazione
% Misure iniziali
misureRange = sqrt((xVett(1)-cTag(:,1)).^2+(yVett(1)-cTag(:,2)).^2) + sigmaDistanza*NNrange(:,1);
...
for k = 1:nPassi-1
    ...
    if mod(k+1,Nstep) == 1 || (Nstep == 1) 
        ...
        misureRange = sqrt((xVett(k+1)-cTag(:,1)).^2+(yVett(k+1)-cTag(:,2)).^2) + sigmaDistanza*NNrange(:,k+1);
        ...
        for indTag = 1:nTag
            ...
            for jndPhi = 1:nIpoTag(indTag)
                KalmanGain(indiceStartIndTag+3+jndPhi,:) = zeros(1,nMisure);
            end
        end
        xTemp = xHatSLAMmeno + KalmanGain*innovazione;
        Ptemp = (eye(dimStato) - KalmanGain*H)*Pmeno*(eye(dimStato) - (KalmanGain*H)') + KalmanGain*Rs*KalmanGain';

        % Ora correggo le ipotesi dei landmark
        H = zeros(nMisure,dimStato); % Jacobiana delle misure di range dh/dx
        innovazione = zeros(nMisure,1);
        Rc = sigmaDistanzaModello^2*eye(nMisure);
        ...
        % Aggiornamento stima (stima a posteriori)
        KalmanGain = Ptemp*H'*pinv(H*Ptemp*H'+Rc);
        KalmanGain(1:3,:) = zeros(3,nMisure);
        for indTag = 1:nTag
            indiceStartIndTag = 3+3*(indTag-1)+sum(nIpoTag(1:indTag-1));
            for jndPhi = 1:nIpoTag(indTag)
                KalmanGain(indiceStartIndTag+1:indiceStartIndTag+3,:) = zeros(3,nMisure);
            end
        end
        xHatSLAM(1:dimStato,k+1) = xTemp + KalmanGain*innovazione;
        P = (eye(dimStato) - KalmanGain*H)*Ptemp*(eye(dimStato) - (KalmanGain*H)') + KalmanGain*Rc*KalmanGain';
...
%             pause(.01)
    ...
    % Calcolo stima complessiva landmark
    for indTag = 1:nTag
        indiceStartIndTag = 3+3*(indTag-1)+sum(nIpoTag(1:indTag-1));
        x_i = xHatSLAM(indiceStartIndTag+1,k+1);
        y_i = xHatSLAM(indiceStartIndTag+2,k+1);
