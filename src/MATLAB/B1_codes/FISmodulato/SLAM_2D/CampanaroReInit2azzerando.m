% Differisce da CampanaroReInit2.m essenzialmente per il fatto che la media
% dell'innovazione non viene calcolata sempre (cosa che potrebbe causare
% reinizializzazioni multiple a cascata finche' la deriva dell'innovazione
% che ha causato la reinizializzazione non esce dalla finestra), ma solo
% a partire da tot passi dopo la reinizializzazione.
...
for k = 1:nPassi-1
    ...
    if mod(k+1,Nstep) == 1 || (Nstep == 1) 
        ...
        for indTag = 1:nTag
            passoMin = max(KreInit(indTag),k-49);
            mediaInn = mean(storiaInnovazione(indTag,passoMin:k+1));
            if abs(mediaInn) > 10 && k > KreInit(indTag)+49
                KreInit(indTag) = k;
