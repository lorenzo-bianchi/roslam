...
% sogliaInnovazione = 1000;%.3;

load roslam_data3.mat
load gt_test7.mat

% IE = 0;%200; % inizio effettivo, cioe' e' il passo da cui far partire la simulazione (da scegliere pari)

nPhi = 8;
nPassi = 1654-IE;
% robotScelto = 2; % e' il robot dei tre che si vuole testare
...
KRvera = 0.01;%05;   % Costante KR errori Odometrici ruota destra: uR_e = N(uR, KR |uR|)
...
if robotScelto == 1
    tempiGT = robot1(:,1) + 11.1; % per allinearli con tempi misure
    xGT = 100*robot1(:,2);
    yGT = 100*robot1(:,3);
    thetaGT = robot1(:,4);
end
if robotScelto == 2
    tempiGT = robot2(:,1) + 11; % per allinearli con tempi misure
    xGT = 100*robot2(:,2);
    yGT = 100*robot2(:,3);
    thetaGT = robot2(:,4);
end
if robotScelto == 3
    tempiGT = robot3(:,1) + 11; % per allinearli con tempi misure
    xGT = 100*robot3(:,2);
    yGT = 100*robot3(:,3);
    thetaGT = robot3(:,4);
end
...
if robotScelto == 1
    t0 = tempiMisure(1);
    [mm,indMin] = min(abs(t0-tempiGT));
    xVett(1) = xGT(indMin); %0.0905*100;
    yVett(1) = yGT(indMin); %2.6885*100;
    thetaVett(1) = 0; %thetaGT(indMin); 
...
if robotScelto == 2
    t0 = tempiMisure(1);
    [mm,indMin] = min(abs(t0-tempiGT));
    xVett(1) =  xGT(indMin); %0.0605*100;
    yVett(1) =  yGT(indMin); %1.6437*100;
    thetaVett(1) = -pi/2; %thetaGT(indMin);
...
if robotScelto == 3
    t0 = tempiMisure(1);
    [mm,indMin] = min(abs(t0-tempiGT));
    xVett(1) = xGT(indMin); %1.7309*100;
    yVett(1) = yGT(indMin); %0.7688*100;
    thetaVett(1) = pi/2;%thetaGT(indMin);
    uRe = 100*odoRuote(IE+21:1654+20,2);
    uLe = 100*odoRuote(IE+21:1654+20,3);
end
...
%         kMisura = kMisura + 1;
%     end
%     [mm,indMin] = min(abs(tempiMisure(kMisura)-tempiGT));
%     xVett(k+1) = xGT(indMin);
%     yVett(k+1) = yGT(indMin);
% end
...
for k = 1:nPassi-2
    if mod(k+1,Nstep) == 0
        xVett(k+1) = (xVett(k)+xVett(k+2))/2;
        yVett(k+1) = (yVett(k)+yVett(k+2))/2;
    end
end
xVett(nPassi) = xVett(nPassi-1);
yVett(nPassi) = yVett(nPassi-1);
...
for k = 1:nPassi-1
    ...
    if mod(k+1,Nstep) == 1 || (Nstep == 1) 
        ...
        nMisure = sum(nIpoTag);
        H = zeros(nMisure,dimStato); % Jacobiana delle misure di range dh/dx
        ...
        for indTag = 1:nTag
            ...
            x_i = xHatSLAMmeno(indiceStartIndTag+1);
            y_i = xHatSLAMmeno(indiceStartIndTag+2);
            rho_i = xHatSLAMmeno(indiceStartIndTag+3);
            probMisura_ij = zeros(nIpoTag(indTag),1);
            for jndPhi = 1:nIpoTag(indTag)
                phi_ij = xHatSLAMmeno(indiceStartIndTag+3+jndPhi); % era 6+(3+nPhi)*(indTag-1)+jndPhi
                ...
                if abs(deltaMisura_ij/misuraRange_ij) > sogliaInnovazione
                    innovazione(indiceMisura) = NaN;
                end
            ...
            for jndPhi = 1:nIpoTag(indTag)
                indiceMisura = sum(nIpoTag(1:indTag-1))+jndPhi;
                if isnan(innovazione(indiceMisura))
                    innovazione(indiceMisura) = 0;
                    Rs(indiceMisura,indiceMisura) = 1000000;
                else
        ...
        xHatSLAM(1:dimStato,k+1) = xHatSLAMmeno + KalmanGain*innovazione;
        P = (eye(dimStato) - KalmanGain*H)*Pmeno;
