...
nMisureTutte = numel(misureTutte(1,:));

for indTag = 1:nTag
    for indMisura = 1:nMisureTutte
            if misureTutte(indTag,indMisura) < 180
                misureTutte(indTag,indMisura) = misureTutte(indTag,indMisura) + 3;
            else
                if misureTutte(indTag,indMisura) > 200
                    misureTutte(indTag,indMisura) = misureTutte(indTag,indMisura) - 7;
                end
            end
    end
end
...
if robotScelto == 1
    ...
    xVett(1) = 0.0905*100; %xGT(indMin);
    yVett(1) = 2.6885*100; %yGT(indMin);
...
if robotScelto == 2
    t0 = tempiMisure(1);
    [mm,indMin] = min(abs(t0-tempiGT));
    xVett(1) =  0.0605*100; %xGT(indMin);
    yVett(1) =  1.6437*100; %yGT(indMin);
...
if robotScelto == 3
    t0 = tempiMisure(1);
    [mm,indMin] = min(abs(t0-tempiGT));
    xVett(1) = 1.7309*100; %xGT(indMin);
    yVett(1) = 0.7688*100; %yGT(indMin);
    thetaVett(1) = pi/2;%thetaGT(indMin);
    uRe = 100*odoRuote(IE+21:1654+20,2);
    uLe = 100*odoRuote(IE+21:1654+20,3);
end
