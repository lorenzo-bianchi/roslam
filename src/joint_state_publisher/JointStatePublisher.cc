...
//////////////////////////////////////////////////
JointStatePublisher::JointStatePublisher() : System() {}
    ...
    EntityComponentManager &_ecm, EventManager &)
{
  ...
  double odomFreq = _sdf->Get<double>("joint_publish_frequency", 50).first;
  if (odomFreq > 0)
  {
    std::chrono::duration<double> period{1 / odomFreq};
    this->odomPubPeriod = std::chrono::duration_cast<std::chrono::steady_clock::duration>(period);
  }

  // If joint_name is specified only those joints. Otherwise, publish all the joints.
    ...
    Entity _joint)
{
  ...
  if (!_ecm.EntityHasComponentType(_joint, components::JointPosition().TypeId()))
  {
    _ecm.CreateComponent(_joint, components::JointPosition());
  }

  // Create joint velocity component if one doesn't exist
  if (!_ecm.EntityHasComponentType(_joint, components::JointVelocity().TypeId()))
  {
                                     ...
                                     const EntityComponentManager &_ecm)
{
  ...
  if (!this->modelPub)
  {
    ...
    if (parentEntity)
    {
      worldName = _ecm.Component<components::Name>(parentEntity->Data())->Data();
      ...
      std::string topicStr =
          topicFromScopedName(this->model.Entity(), _ecm, false) + "/joint_state";
  ...
  // Set the model pose
  const auto *pose = _ecm.Component<components::Pose>(this->model.Entity());
  if (pose)
  ...
  for (const Entity &joint : this->joints)
  {
    ...
    const auto *jointPositions  = _ecm.Component<components::JointPosition>(joint);
    if (jointPositions)
    {
    ...
    const auto *jointVelocity = _ecm.Component<components::JointVelocity>(joint);
    if (jointVelocity)
    {
    ...
    // // Set the joint force
    // const auto *jointForce  = _ecm.Component<components::JointForce>(joint);
    // if (jointForce)
    // {
...
IGNITION_ADD_PLUGIN_ALIAS(JointStatePublisher, "custom::JointStatePublisher")
